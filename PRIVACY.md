...
Only **COMMONS / NOW** makes live public-data requests. The Museum entrance, **DEEP SPACE / ALMOST**, **ALMOST ONLINE!**, **PAGE FOUR**, and **ELSEWHERE / CATALOGUE 0** do not initiate data-service requests. PAGE FOUR contains a fictional archive plus separately labeled real-source research and instrument layers; CATALOGUE 0 is explicitly fictional. Neither space accepts visitor stories, names, submissions, or uploads.
...
- create cookies;
- write visitor profiles or histories to `localStorage`, `sessionStorage`, or IndexedDB;
...
- **PAGE FOUR** keeps three evidence modes distinct. The original archive case files remain fictional. The Evidence Lattice and Hessdalen Instrument Room use documented real-source material and methodological controls, while any Page Four hypothesis remains explicitly authored speculation. External source URLs live in repository documentation, not visitor runtime code.
...
Ordinary browser navigation, URL fragments, and local clipboard use are not treated as application telemetry. The Museum does not record which route or Page Four file a visitor chooses.
