(function attachCosmicSignal(root, factory) {
  const api = factory();
  if (typeof module === 'object' && module.exports) module.exports = api;
  root.MuseumCosmicSignal = api;
  if (root.document) api.mount(root.document, root);
})(typeof globalThis !== 'undefined' ? globalThis : this, function buildCosmicSignal() {
  'use strict';
  ...
  function parseScale(value) {
    ...
    if (!Number.isInteger(number) || number < 0 || number > 5) return null;
    return number;
  }
  ...
  function cosmicSentence(solarText, scales) {
    ...
    if (scales?.geomagnetic?.available) {
      parts.push(`geomagnetic ${scales.geomagnetic.code} ${scales.geomagnetic.text}`);
    }
    if (scales?.radiation?.available) {
      parts.push(`solar radiation ${scales.radiation.code} ${scales.radiation.text}`);
    }
    if (!parts.length) return 'Cosmic measurements are unavailable in this snapshot.';
    return `${parts.join(' · ')}.`;
  }
