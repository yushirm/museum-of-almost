...
(() => {
  ...
  const installReturnCart = () => {
    ...
    controls.innerHTML = '<strong>RETURN CART · EMPTY</strong><span>Open an accession directly. When you move to another record, the previous object waits here to be reshelved.</span>';
    const style = document.createElement('style');
  ...
  const installFreightLiftConsequence = () => {
    ...
    style.textContent = `
      ...
      @media(max-width:620px){.lift-service-state{grid-template-columns:1fr}.lift-service-toggle{justify-self:start}.corridor.has-lift-service-scar .freight-lift:not(.is-out-of-service)::after{right:.25rem;bottom:-1.25rem}}@media(prefers-contrast:more){.lift-service-state,.lift-service-toggle{border-width:2px}.freight-lift.is-out-of-service{outline-width:4px}}@media(prefers-reduced-motion:reduce){.corridor.has-lift-service-scar .freight-lift:not(.is-out-of-service)::after{transform:none}}@media print{.lift-service-toggle{display:none}.lift-consequence{border-left-width:2px}.corridor.has-lift-service-scar .freight-lift::after,.corridor.has-lift-service-scar .corridor-note::after{display:none}}
    `;
    document.head.append(style);
