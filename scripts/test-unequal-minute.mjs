...
assert.ok(core.STATIONS.every(({ radiusRatio }) => radiusRatio > 1));
...
for (const station of core.STATIONS) {
  ...
  assert.ok(one.properStepSeconds > previousStep);
  assert.ok(one.properStepSeconds < 60);
...
assert.equal(JSON.stringify(core.STATIONS), declarationsBefore);
...
assert.equal((viewSource.match(/make\('button'/g) || []).length, 1);
...
console.log('Unequal Minute fixed-radius Schwarzschild lapse math, one-command unequal proper-time increments, horizon/free-fall boundary, accessibility, privacy, deterministic mount, v41 preservation, and v42 offline shell verified.');