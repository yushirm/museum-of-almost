...
for (const source of [html, css, teaser, js, researchJs, researchCss]) {
...
assert.doesNotMatch([js, researchJs].join('\n'), /\bfetch\s*\(|XMLHttpRequest|sendBeacon|WebSocket|EventSource/i, 'Page Four must not add runtime network requests');
assert.doesNotMatch([js, researchJs].join('\n'), /localStorage|sessionStorage|indexedDB|document\.cookie|navigator\.geolocation/i, 'Page Four must not persist or personalize visitor data');
assert.doesNotMatch([js, researchJs].join('\n'), /analytics|telemetry|tracking|gtag|dataLayer|mixpanel|segment|plausible|amplitude|hotjar/i, 'Page Four must remain tracking-free');
assert.doesNotMatch([js, researchJs].join('\n'), /navigator\.share\b/, 'Page Four must not hand data to external share targets automatically');
assert.doesNotMatch(researchJs, /innerHTML|insertAdjacentHTML|document\.write/i, 'research evidence must mount through DOM nodes rather than HTML-string injection');
...
assert.match(js, /loadResearchWing\(\)/, 'Page Four should progressively mount the research wing');
...
for (const styles of [css, teaser, researchCss]) {
...
console.log('Page Four fictional archive, sourced evidence lattice, local leak desk, static sighting log, shared permalinks, accessibility, no-network boundary, evidence links, and entrance reveal verified.');
