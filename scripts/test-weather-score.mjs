...
const source = await import('node:fs/promises').then((fs) => fs.readFile(new URL('../weather-score.js', import.meta.url), 'utf8'));
assert.match(source, /AudioContext/);
assert.match(source, /Play this latch/);
assert.match(source, /not environmental sound/);
assert.match(source, /missing temperature becomes a rest|missing temperatures remain silent rests/i);
assert.doesNotMatch(source, /fetch\s*\(/);
assert.doesNotMatch(source, /localStorage|sessionStorage|indexedDB|geolocation/i);
