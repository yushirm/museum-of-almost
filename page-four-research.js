...
(() => {
  ...
  function mountExaminationLightTable(researchSection) {
    ...
    const examTitle = text('h2', 'THREE LIGHTS. ONE FRAGMENT.', 'examination-title');
    examTitle.id = 'examination-light-table-title';
    examHeading.append(
    ...
    function visualSpan(className, value = '') {
    ...
    function fragmentLine(parts) {
      const paragraph = document.createElement('p');
      parts.forEach((part) => paragraph.append(typeof part === 'string' ? document.createTextNode(part) : part));
      return paragraph;
    }

    const watermark = visualSpan('exam-watermark', 'ARCHIVE STOCK\n— 04 —');
    sheet.append(
      visualSpan('exam-fold'),
      visualSpan('exam-pinhole exam-pinhole-a'),
      visualSpan('exam-pinhole exam-pinhole-b'),
      visualSpan('exam-pressure exam-pressure-a'),
      visualSpan('exam-pressure exam-pressure-b'),
      watermark,
      text('small', 'DOCUMENT ID: FOUR-08 / RECONSTRUCTION'),
      fragmentLine(['Subject exhibits ', redact(), ' when exposed to ', redact(), '.']),
      fragmentLine(['Secondary effects include ', redact(), ' and ', redact(), '.']),
      fragmentLine(['Recommended action: ', redact(), '.']),
      fragmentLine(['Do not ', redact(), ' under any circumstances.'])
    );
    stage.append(sheet);
